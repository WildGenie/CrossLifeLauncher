...
namespace Patcher
{
    public partial class frmMain : Form
    {
        ...
        void Update(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                currentVersion = Updater.GetCurrentVersion(Path.Combine("version.txt"));
            }
            catch (Exception ex)
            {
                e.Cancel = true;
                Program.ravenClient.CaptureException(ex);
            ...
            {
                e.Cancel = true;
                MessageBox.Show(string.Format("`{0}` returned an invalid response. Integer is expected.", configuration.VersionUrl), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            catch (Exception ex)
            {
            ...
            for (int i = currentVersion; i < lastVersion; i++)
            {
                ...
                if (downloadingException != null)
                {
                    ...
                    HttpWebResponse response = (HttpWebResponse)webex.Response;
                    e.Cancel = true;
                    Program.ravenClient.CaptureException(webex);
